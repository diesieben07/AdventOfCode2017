 ...
 * @author Take Weiland
 */
val testData = "5 1 9 5\n" + "7 5 3\n" + "2 4 6 8"
val data = "1224\t926\t1380\t688\t845\t109\t118\t88\t1275\t1306\t91\t796\t102\t1361\t27\t995\n" +
...
fun parseSpreadsheet(input: String): List<List<Int>> {
    return input.split(Regex("[\\r\\n]+"))
            .map { row -> row.trim().split(Regex("\\s+")).map { it.trim().toInt() } }
}

fun rowChecksum(row: List<Int>): Int {
    val statistics = row.stream().mapToInt { i -> i }
            .summaryStatistics()
